...
#include "SDL_ttf.h"
using namespace std;
//_______________GameStruct__________________________________________________________
SDL_Window* win = NULL;
...
enum class GameState { NULLER, MainMenu, GamePlay, GameOver,HelpPage,Quit};
//建议改名为flag
union Type
{
    //声明部分
...
public:
    ...
    virtual void FunDraw(PhyJson* drawptr,SDL_Texture* _texture)
    {
        cout << "Havnt override!" << endl;
    }
...
public:
    DrawComp(PhyJson* jsonptr, SDL_Texture* texture,
        void (*_Draw_Fun)(PhyJson* drawptr, SDL_Texture* _texture)) :
    ...
    void FunDraw(PhyJson* drawptr, SDL_Texture* _texture) override
    {
        if (Draw_Fun)
            Draw_Fun(Phy_Comp, mtexture);
    }
    SDL_Texture* mtexture;
private:
    void (*Draw_Fun)(PhyJson* drawptr, SDL_Texture* _texture);
};

void Stand_Draw_Fun(PhyJson* drawptr, SDL_Texture* _texture)
{
    int ScreenX = drawptr->center.x - drawptr->width / 2;
    int ScreenY = drawptr->center.y - drawptr->height / 2;
    SDL_Rect destRect = { ScreenX, ScreenY,drawptr->width,drawptr->height };
    SDL_RenderCopyEx(renderer, _texture, nullptr, &destRect,
        drawptr->angle, nullptr, SDL_FLIP_NONE);
    return;
}
...
public:
    ...
    void ActorOutPut()
    {
        ...
        for (auto iter : UpTag)
        {
            iter->FunDraw(ActorJson,dynamic_cast<DrawComp*>(iter)->mtexture);
        }
...
SDL_Color VS_White = { 255,255,255 };
SDL_Color VS_Yellow = {253,249,43};
int WinGames = 3;
...
void Paddle_Phy(PhyJson* jsonptr, float deltaime) {
    ...
    jsonptr->center.y += (int)(jsonptr->velocity.y * deltaime);
    //板的限制
    if (jsonptr->center.y < Paddlength / 2)
    {
        jsonptr->center.y = Paddlength / 2 ;
    }
    if (jsonptr->center.y > WINHEIGHT - Paddlength / 2)
    {
        jsonptr->center.y = WINHEIGHT - Paddlength / 2 ;
    }
}
...
Actor* Ballnit()
{
    ...
    BusPipe.Add(Ballptr);
    srand(SDL_GetTicks());
    Ballptr->velocity = { rand()%400-200,rand()%400-200};
    //最小速度判断
    if (abs(Ballptr->velocity.x) < 100)
    {
        if (Ballptr->velocity.x == 0)
            Ballptr->velocity.x = 100;
        else
        {
            Ballptr->velocity.x = 164;
        }
    }
    if (abs(Ballptr->velocity.y) < 100)
    {
        if (Ballptr->velocity.y == 0)
            Ballptr->velocity.y = 100;
        else
        {
            Ballptr->velocity.y = 185;
        }
    }
    Ballptr->center = { WINWIDTH / 2, WINHEIGHT / 2 };
    Ballptr->name = "Ball";
    ...
    Comp* Ball_Draw = new DrawComp(Ballptr, texture,Stand_Draw_Fun);
...
Actor* PaddleInit(Type::_PADDLETYPE ptype, PhyJson* tagetjson)
{
    ...
    texture = SDL_CreateTextureFromSurface(renderer, surf);
    Comp* PaddleDrawer = new DrawComp(Paddle, texture,Stand_Draw_Fun);
...
//被状态量激发的game状态转换还是得用触发器来做，因为PHY自动填入的是actor的json
...
//基于按钮的状态转换
void Stand_Button_Eventhandler(SDL_Event* event, PhyJson* TagetPhy)
{
    SDL_Point mousep = { event->motion.x,event->motion.y };
    switch (event->type)
    {
    case SDL_MOUSEMOTION:        
        if (SDL_PointInRect(&mousep, &(TagetPhy->Json_Type.button.rect)))
        {
            TagetPhy->Json_Type.button.nowColor =
                TagetPhy->Json_Type.button.inColor;
        }
        else
        {
            TagetPhy->Json_Type.button.nowColor =
                TagetPhy->Json_Type.button.OutColor;
        }
        break;
    case SDL_MOUSEBUTTONDOWN:
        //这里还可以抽象一下，可以把函数指针用进来，这样复用性就很高        
        if (SDL_PointInRect(&mousep, &(TagetPhy->Json_Type.button.rect)))
        {
...
void Begin_Button_Eventhandler(SDL_Event* event, PhyJson* TagetPhy)
{
    SDL_Point mousep = { event->motion.x,event->motion.y };
    switch (event->type)
    {
...
void Help_Button_Eventhandler(SDL_Event* event, PhyJson* TagetPhy)
{
    SDL_Point mousep = { event->motion.x,event->motion.y };
    switch (event->type)
    {
...
void Stand_Button_Drawer(PhyJson* drawptr, SDL_Texture* _texture)
{
    if (_texture == NULL)//如果不提供资源，则
    {
        surf = SDL_CreateRGBSurface(0, drawptr->width, drawptr->height, 32, 0, 0, 0, 0);
        SDL_Color temp = drawptr->Json_Type.button.nowColor;
        SDL_FillRect(surf, NULL, SDL_MapRGB(surf->format,temp.r,temp.g,temp.b));
        texture = SDL_CreateTextureFromSurface(renderer, surf);
        int ScreenX = drawptr->center.x - drawptr->width / 2;
        int ScreenY = drawptr->center.y - drawptr->height / 2;
        SDL_Rect destRect = { ScreenX, ScreenY,drawptr->width,drawptr->height };
        ...
        SDL_RenderDrawRect(renderer, &(drawptr->Json_Type.button.rect));
    }
    else
    {
...
Actor* ButtonInit(PhyJson* jsonptr,string Title,
                  void (*EventHandler)(SDL_Event* event, PhyJson* TagetPhy))
{
    Actor* Button = new Actor(jsonptr);
    //下面需要一个更新mexture的eventhandler组件
    Comp* Button_Trigger = new EventWatcher(jsonptr, EventHandler);
    Comp* Button_Drawer = new DrawComp(jsonptr, NULL, Stand_Button_Drawer);
    Comp* Button_Texter = new TextComp(jsonptr,Title,VS_White);
    Button->CompStore.push_back(Button_Drawer);
    Button->CompStore.push_back(Button_Trigger);
    Button->CompStore.push_back(Button_Texter);
    return Button;
}

//配套的json指针生成函数:对于每个函数可以写不一样的
PhyJson* Title_Button_Gen_Phyjson(SDL_Color inco, SDL_Color outco,
                                    SDL_Point center)
{
    PhyJson* buttonptr = new PhyJson;
    buttonptr->name = "Title_Button_Top_Star";
    buttonptr->center.x = center.x;
    buttonptr->center.y = center.y;
    buttonptr->width = 200;
    buttonptr->height = 70;
    buttonptr->Json_Type.button.inColor = inco;
    buttonptr->Json_Type.button.OutColor = outco;
    buttonptr->Json_Type.button.nowColor = outco;
    buttonptr->Json_Type.button.rect = 
    { buttonptr->center.x - buttonptr->width / 2,
      buttonptr->center.y - buttonptr->height / 2,
      buttonptr->width,buttonptr->height
    };
    buttonptr->Json_Type.button.button_trigger = false;
    return buttonptr;
}

void Title_Phy_Fun(PhyJson* title_jp,float deltatime)
{
    //在+5和-5之间摇摆
    if (title_jp->RotationSpeed > 0)
    {
        if (title_jp->angle < 5.0f)
        {
            title_jp->angle += title_jp->RotationSpeed * deltatime;
        }else
        { 
            title_jp->RotationSpeed *= -1;
            return;
        }
    }
    if (title_jp->RotationSpeed < 0)
...
Actor* TitleInit()
{
    PhyJson* title = new PhyJson;
    Actor* _title = new Actor(title);
    title->name = "GameTitle";
    title->center = {WINWIDTH/2,70};
    title->Rotated = true;
    title->RotationSpeed = 10;
    Comp* title_Phy = new Physics(title,Title_Phy_Fun);
    _title->CompStore.push_back(title_Phy);
    Comp* title_texter = new TextComp(title, "PIN PONG!", VS_Yellow);
    _title->CompStore.push_back(title_texter);
    
    return _title;
}
//————————————————————————HelpPage对象层——————————————————————————————
void Back_Button_Eventhandler(SDL_Event* event, PhyJson* TagetPhy)
{
    SDL_Point mousep = { event->motion.x,event->motion.y };
    switch (event->type)
    {
...
void TitleInit(Game* game)
{
    ...
    game->SpriteStore.push_back(backround);
    Actor* Button_1 = ButtonInit(Title_Button_Gen_Phyjson(VS_Yellow,VS_BLUE,
        { WINWIDTH / 2,WINHEIGHT / 2 })," Start ",
        Begin_Button_Eventhandler);
    game->mActors.push_back(Button_1);
    Actor* Button_2 = ButtonInit(Title_Button_Gen_Phyjson(VS_Yellow, VS_BLUE, 
        { WINWIDTH / 2,WINHEIGHT / 2 + 90 }),"Help",
        Help_Button_Eventhandler);
    game->mActors.push_back(Button_2);
    game->mActors.push_back(TitleInit());
}

void HelpInit(Game* game)
{
    game->GClear();
    SpriteComp* backround = BackRoundSpriteInit("help.png");
    game->SpriteStore.push_back(backround);
    Actor* Back_button = ButtonInit(Title_Button_Gen_Phyjson(VS_Yellow, VS_BLUE,
        { 690,360 }), "Back",
        Back_Button_Eventhandler);
    game->mActors.push_back(Back_button);
}
...
int main(int argc, char* argv[])
{
    ...
    GameState lastState = GameState::NULLER;//防止重复装载影响运行
    while (true)
    {
